...
interface POIDistance extends SurvivalPOI {
  distance: number;
}
...
export default function SurvivalKitMap({ activeCategories, selectedCity }: Props) {
  ...
  const locate = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocError("No se pudo obtener tu ubicacion. Activa los permisos.");
        setLocating(false);
      },
    ...
    <div>
      <div className="rounded-xl overflow-hidden border border-gray-200 shadow-sm relative">
        ...
        <MapContainer center={center} zoom={zoom} style={{ height: "450px", width: "100%" }} scrollWheelZoom={true} className="z-0">
          ...
          {!userPos && <FitBounds points={filteredPOIs.map((p) => [p.lat, p.lng] as [number, number])} />}
            ...
            <Marker key={poi.id} position={[poi.lat, poi.lng]} icon={createPOIIcon(poi.category)}>
              <Popup>
                <div style={{ fontFamily: "system-ui", minWidth: "220px" }}>
                  ...
                  <div style={{ fontSize: "11px", color: "#999", marginBottom: "6px" }}>{cityNames[poi.city] || poi.city}</div>
                  {userPos && (
      ...
      {!userPos && (
        <div className="mt-4 bg-blue-50 rounded-xl border border-blue-200 p-4 text-center">
          <p className="text-sm text-blue-800">
            <strong>Toca "Dónde estoy"</strong> para ver los puntos de interes mas cercanos a tu ubicacion actual y obtener direcciones.
          </p>
        </div>
      )}
